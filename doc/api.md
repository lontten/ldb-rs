...
```rust
use ldb::{
    connect_mysql, connect_pg, count, delete, first, get_or_insert, has, insert, list,
    update, update_by_primary_key, w,
...
| `primary_key` | `fn primary_key(self, id: impl IntoSqlValue) -> Self` | 按主键等值查询（单主键） |
| `filter_primary_key` | `fn filter_primary_key(self) -> Self` | 生成主键非空过滤（用于 update 等） |
...
| 方法 | 说明 |
|------|------|
| `set_null` / `set` / `set_increment` / `set_expression` | 额外 SET 子句，与 `patch` 字段合并 |
| 共用方法 | `table_name`、`show_sql`、`dry_run` |
...
| `set_null` / `set` / `set_increment` / `set_expression` | 额外 SET 子句 |
| 共用方法 | `table_name`、`show_sql`、`dry_run` |
...
| CRUD 入口 | `insert`, `update`, `update_by_primary_key`, `delete`, `first`, `list`, `has`, `count`, `get_or_insert`, `InsertResult` |
| 条件 | `w`, `WhereBuilder` |
| 枚举 | `OnConflict`, `Order` |
| 错误 | `LdbError` |
